...
# from src.model_stuff.downstream_model_cnn import MyDownstreamModel
from src.model_stuff.downstream_model_cnn_bn import MyDownstreamModel
...
EXP_NAME = f"CNN_BN_{ON_SERVER}_downstrexp_fe{fe}_gs{gs}_bs{bs}_lr{lr}_drpout{drpout}_freeze{freeze}_nFC{nFC}_num_out_neurons{num_out_neurons}"
...
checkpoint_callback = ModelCheckpoint(
    # dirpath=f'./saved_models/downstream/{EXP_NAME}',
    dirpath=f'/workspace/repos/hrdl/saved_models/avg100ep_2class/downstream_CNN_BN/',
    filename='{epoch}-{val_majority_vote_acc:.3f}-{val_acc_epoch:.3f}',
    verbose=True,
    monitor='epoch',
    mode='max'
